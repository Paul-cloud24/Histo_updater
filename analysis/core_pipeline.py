...
class CoreStainPipeline:
        ...
        if use_stardist:
            try:
                ...
                if CoreStainPipeline._stardist_model is None:
                    ...
                    print(f"[{stain_name}] StarDist aus Cache ✔")
                self.stardist_model = CoreStainPipeline._stardist_model
            except ImportError:
                print(f"[{stain_name}] ⚠ StarDist nicht installiert → Watershed")
                self.use_stardist = False
            except OSError as e:
                if "1314" in str(e) or "symlink" in str(e).lower():
                    print(f"[{stain_name}] ⚠ Symlink nicht erlaubt → Watershed")
                    self.use_stardist = False
                    self.stardist_model = None
                else:
                    raise
    ...
    def run(self, marker_path: str, dapi_path: str,
        ...
        out_overlay = os.path.join(output_folder, "Overlays")
        out_summary = os.path.join(output_folder, "Summary")
        ...
        # 5) Export – in Unterordner
        try:
            csv = self._save_csv(df, out_summary, base)
        except Exception as e:
            print(f"  ⚠ CSV konnte nicht gespeichert werden: {e}")
            csv = None
        try:
            qc = self._save_qc(df, out_summary, base)
        except Exception as e:
            print(f"  ⚠ QC-Plot konnte nicht gespeichert werden: {e}")
            qc = None
        try:
            ov = self._save_overlay(marker_img, dapi_masks, df,
                                    out_overlay, base)
        except Exception as e:
            print(f"  ⚠ Overlay konnte nicht gespeichert werden: {e}")
            ov = None
        ...
        try:
            ...
            summary.to_csv(
                os.path.join(out_summary, f"{base}_summary.csv"),
                index=False
            )
        except Exception as e:
            print(f"  ⚠ Summary CSV Fehler: {e}")
