...
class HistologyUI(QMainWindow):
    def __init__(self):
        ...
        self.btn_update.setObjectName("warning")
        ...
        lbl_sub  = QLabel("ANALYZER  ·  v2.1"); lbl_sub.setObjectName("subheader")
    ...
    def _open_roi_dialog(self):
        if not self.folder:
            QMessageBox.warning(self, "Fehler", "Bitte zuerst Ordner wählen.")
            return
        roi_root = os.path.join(self.folder, "Results", "ROIs")
       ...
       # Bild-Pfade für alle Ordner ermitteln
        ...
        dlg = stain.get_roi_dialog(img_paths[0], parent=self, roi_root=roi_root)
        if dlg is None:
            QMessageBox.information(self, "Info",
    ...
    def _on_adjust_roi_clicked(self):
        ...
        dlg = ROIDialog(img_path, parent=self, roi_root=roi_root)
        ...
        # ROIDialog hat kein roi_skipped — rejected() wenn Abbrechen geklickt
        dlg.rejected.connect(lambda: self._advance_to_next_folder())
        dlg.exec()
    ...
    def _process_next_auto_roi(self):
        ...
        timer = self._pipeline_params.get("auto_roi_timer", 7)
        self._start_countdown(img_path, polygon, seconds=timer)
    ...
    def _run_auto_roi_batch(self):
        ...
        for img_path, polygon in self._auto_roi_results.items():
            ...
            json_path = os.path.join(roi_root, f"{folder_name}_roi.json")
    ...
    def _start_update_check(self):
        print("[Update] _start_update_check aufgerufen")
        try:
            from ui.updater import UpdateChecker
            import platform

            # Version aus Override-Ordner lesen falls vorhanden
            if platform.system() == "Windows":
                import sys, os
                exe_dir = os.path.dirname(os.path.abspath(sys.argv[0]))
                override_version = os.path.join(
                    exe_dir, "Histo_analyzer", "version.py")
                if os.path.exists(override_version):
                    import importlib.util
                    spec = importlib.util.spec_from_file_location(
                        "version", override_version)
                    ver_mod = importlib.util.load_from_spec(spec)
                    spec.loader.exec_module(ver_mod)
                    VERSION      = ver_mod.VERSION
                    UPDATE_REPO  = ver_mod.UPDATE_REPO
                    UPDATE_BRANCH = ver_mod.UPDATE_BRANCH
                else:
                    from version import VERSION, UPDATE_REPO, UPDATE_BRANCH
            else:
                from version import VERSION, UPDATE_REPO, UPDATE_BRANCH

            print(f"[Update] Version={VERSION} Repo={UPDATE_REPO} Branch={UPDATE_BRANCH}")
            ...
            self._updater.up_to_date.connect(
                lambda: print("[Update] Bereits aktuell"))
            self._updater.error.connect(
                lambda e: print(f"[Update] Fehler: {e}"))
    ...
    def _on_update_clicked(self):
        ...
        if platform.system() == "Windows":
            exe_dir = os.path.dirname(os.path.abspath(sys.argv[0]))
            app_root = os.path.join(exe_dir, "Histo_analyzer")
    ...
    def _start_batch(self, stain_name: str):
        # Alten Worker stoppen falls noch aktiv
        if self._batch_worker is not None:
            print("[Batch] Bereits ein Worker aktiv – warte...")
            return

        # Alle alten Worker beenden
        ...
        self._batch_worker.signals.preview.connect(self._show_preview)
        self._batch_worker.signals.overlay.connect(self._show_overlay_preview)
    ...
    def _on_batch_finished(self, csv_path, plot_path):
        # Cache VOR dem Zurücksetzen holen
        results = getattr(self._batch_worker, '_results_cache', [])
        self._batch_worker = None  # ← DANACH zurücksetzen
        ...
        if results:
            self._open_roi_overview(results)
        else:
            QMessageBox.information(self, "Fertig",
                f"Batch-Analyse abgeschlossen!\n\n"
                f"Ergebnisse in:\n{self.folder}/Results/")

    def _open_roi_overview(self, results: list):
        from ui.roi_overview_dialog import ROIOverviewDialog
        dlg = ROIOverviewDialog(results, parent=self)
        dlg.reanalyse_requested.connect(self._reanalyse_selected)
        dlg.exec()

    def _reanalyse_selected(self, folder_paths: list):
        """ROI für ausgewählte Ordner neu einzeichnen und neu analysieren."""
        from ui.roi_dialog import ROIDialog
        from analysis.core_pipeline import find_images_in_folder

        # DAPI-Bilder für alle ausgewählten Ordner sammeln
        img_paths = []
        for folder_path in folder_paths:
            try:
                _, dapi_path = find_images_in_folder(folder_path)
                img_paths.append(dapi_path)
            except Exception as e:
                self._log(f"⚠ {os.path.basename(folder_path)}: {e}")
        ...
        if dlg.exec() == QDialog.Accepted:
            # Neu analysieren
            self._log("── Neu-Analyse gestartet ──")
            self._use_roi = True
            self._start_batch(stain_name)
